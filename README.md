...
- **Closed type:** `type Decision = PlaceOrder(Catalog) | DeclineOrder` where `Catalog` is closed
- **Sink / capability:** `grant placeOrder irreversible` → `commit { placeOrder(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/shopping-agent/demo.sh
```
...
- `shopping-agent_safe.wave` — the correct design.
- `shopping-agent_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
